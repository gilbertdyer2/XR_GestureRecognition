...
Unfortunately the game itself is not finished as of the most recent commit and also requires a Meta Quest to run, but the model architecture is fully complete and a basic working version has been trained that seems to work well. I've provided a script (visualize_data) in utils/ to visualize the data the model was trained on, and a script to test the model's effectiveness on sample data of drawings of buildings, separately recorded from the dataset (test_trained_fromJSON).
